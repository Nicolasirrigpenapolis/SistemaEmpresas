...
using SistemaEmpresas.Features.Emitentes.Dtos;
using SistemaEmpresas.Models;
using SistemaEmpresas.Core.Services;
using System.Text.Json;

namespace SistemaEmpresas.Features.Configuracoes.Controllers;
...
public class ConfiguracoesController : ControllerBase
{
    ...
    public async Task<ActionResult<EmitenteDto>> GetEmitenteAtual()
    {
        ...
        if (emitente == null)
        {
            return NotFound(new { message = "Nenhum emitente cadastrado para esta empresa. Por favor, configure o emitente." });
        }
    ...
    /// </summary>
    [HttpPost("emitente/{id}/certificado")]
    ...
    /// </summary>
    [HttpGet("emitente/{id}/certificado/validar")]
    ...
    /// </summary>
    [HttpDelete("emitente/{id}/certificado")]
    ...
    /// <summary>
    /// Obtém os parâmetros do sistema (diretórios de fotos, etc.)
    /// </summary>
    [HttpGet("parametros")]
    public async Task<ActionResult<ParametrosDto>> GetParametros()
    {
        ...
        return Ok(new ParametrosDto
        {
            DiretorioFotosProdutos = parametros.DiretorioFotosProdutos,
            DiretorioFotosConjuntos = parametros.DiretorioFotosConjuntos,
            DiretorioDasFotos = parametros.DiretorioDasFotos,
            DiretorioDesenhoTec = parametros.DiretorioDesenhoTec,
            CaminhoAtualizacao = parametros.CaminhoAtualizacao,
            CaminhoAtualizacao2 = parametros.CaminhoAtualizacao2,
            NomeDoServidor = parametros.NomeDoServidor
        });
    }

    /// <summary>
    /// Atualiza os parâmetros do sistema
    /// NOTA: Tabela Parametros é Keyless, então usamos SQL direto
    /// </summary>
    [HttpPut("parametros")]
    public async Task<ActionResult<ParametrosDto>> UpdateParametros(ParametrosUpdateDto dto)
    {
        try
        {
            var parametros = await _context.Parametros.FirstOrDefaultAsync();
            ...
            if (parametros == null)
            {
                // Cria um novo registro de parâmetros se não existir
                await _context.Database.ExecuteSqlRawAsync(@"
                    INSERT INTO [Parâmetros] 
                    ([Diretorio Fotos Produtos], [Diretorio Fotos Conjuntos], [Diretorio das Fotos], 
                     [Diretorio Desenho Tec], [Caminho Atualização], [Caminho Atualização 2], [Nome do Servidor])
                    VALUES ({0}, {1}, {2}, {3}, {4}, {5}, {6})",
                    dto.DiretorioFotosProdutos ?? "",
                    dto.DiretorioFotosConjuntos ?? "",
                    dto.DiretorioDasFotos ?? "",
                    dto.DiretorioDesenhoTec ?? "",
                    dto.CaminhoAtualizacao ?? "",
                    dto.CaminhoAtualizacao2 ?? "",
                    dto.NomeDoServidor ?? "");
            }
            else
            {
                // Atualiza campos usando SQL direto (tabela Keyless não suporta tracking)
                await _context.Database.ExecuteSqlRawAsync(@"
                    UPDATE [Parâmetros] SET 
                        [Diretorio Fotos Produtos] = {0},
                        [Diretorio Fotos Conjuntos] = {1},
                        [Diretorio das Fotos] = {2},
                        [Diretorio Desenho Tec] = {3},
                        [Caminho Atualização] = {4},
                        [Caminho Atualização 2] = {5},
                        [Nome do Servidor] = {6}",
                    dto.DiretorioFotosProdutos ?? parametros.DiretorioFotosProdutos,
                    dto.DiretorioFotosConjuntos ?? parametros.DiretorioFotosConjuntos,
                    dto.DiretorioDasFotos ?? parametros.DiretorioDasFotos,
                    dto.DiretorioDesenhoTec ?? parametros.DiretorioDesenhoTec,
                    dto.CaminhoAtualizacao ?? parametros.CaminhoAtualizacao,
                    dto.CaminhoAtualizacao2 ?? parametros.CaminhoAtualizacao2,
                    dto.NomeDoServidor ?? parametros.NomeDoServidor);
            }

            _logger.LogInformation("Parâmetros do sistema atualizados");

            // Retorna os parâmetros atualizados
            var parametrosAtualizados = await _context.Parametros.FirstOrDefaultAsync();
            
            return Ok(new ParametrosDto
            {
                DiretorioFotosProdutos = parametrosAtualizados?.DiretorioFotosProdutos ?? "",
                DiretorioFotosConjuntos = parametrosAtualizados?.DiretorioFotosConjuntos ?? "",
                DiretorioDasFotos = parametrosAtualizados?.DiretorioDasFotos ?? "",
                DiretorioDesenhoTec = parametrosAtualizados?.DiretorioDesenhoTec ?? "",
                CaminhoAtualizacao = parametrosAtualizados?.CaminhoAtualizacao ?? "",
                CaminhoAtualizacao2 = parametrosAtualizados?.CaminhoAtualizacao2 ?? "",
                NomeDoServidor = parametrosAtualizados?.NomeDoServidor ?? ""
            });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Erro ao atualizar parâmetros do sistema");
            return StatusCode(500, new { message = "Erro ao salvar parâmetros: " + ex.Message });
        }
    }
...
/// <summary>
/// DTO para parâmetros do sistema
/// </summary>
public class ParametrosDto
{
    public string DiretorioFotosProdutos { get; set; } = "";
    public string DiretorioFotosConjuntos { get; set; } = "";
    public string DiretorioDasFotos { get; set; } = "";
    public string DiretorioDesenhoTec { get; set; } = "";
    public string CaminhoAtualizacao { get; set; } = "";
    public string CaminhoAtualizacao2 { get; set; } = "";
    public string NomeDoServidor { get; set; } = "";
}

/// <summary>
/// DTO para atualização de parâmetros do sistema
/// </summary>
public class ParametrosUpdateDto
{
    public string? DiretorioFotosProdutos { get; set; }
    public string? DiretorioFotosConjuntos { get; set; }
    public string? DiretorioDasFotos { get; set; }
