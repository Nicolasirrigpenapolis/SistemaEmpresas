...
using ProdutoModel = SistemaEmpresas.Models.Produto;
...
public interface IProdutoRepository
{
    ...
    Task<ProdutoModel> CriarAsync(ProdutoCreateUpdateDto dto, string usuario);
    Task<ProdutoModel?> AtualizarAsync(int id, ProdutoCreateUpdateDto dto, string usuario);
...
public class ProdutoRepository : IProdutoRepository
{
    ...
    public async Task<PagedResult<ProdutoListDto>> ListarAsync(ProdutoFiltroDto filtro)
    {
        ...
        {
            query = query.Where(p => p.QuantidadeContabil < p.QuantidadeMinima && p.QuantidadeMinima > 0);
        }
    ...
    public async Task<ProdutoModel?> AtualizarAsync(int id, ProdutoCreateUpdateDto dto, string usuario)
    {
    ...
    public async Task<List<ProdutoComboDto>> ListarParaComboAsync(string? busca = null)
    {
        ...
        return await query
            ...
            .Select(p => new ProdutoComboDto
            {
                ...
                Unidade = p.SequenciaDaUnidadeNavigation.Descricao,
                ValorTotal = p.ValorTotal,
                QuantidadeNoEstoque = p.QuantidadeContabil // Usar estoque contábil
            })
            .ToListAsync();
    }
    ...
    public async Task<int> ContarEstoqueCriticoAsync()
    {
        return await _context.Produtos
            .Where(p => !p.Inativo && p.QuantidadeContabil < p.QuantidadeMinima && p.QuantidadeMinima > 0)
            .CountAsync();
    }
}
